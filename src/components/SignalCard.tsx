...
export default function SignalCard({ signal }: { signal: any }) {
  ...
  return (
    <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden hover:border-slate-700 transition-all">
      <div className={`bg-gradient-to-r ${getSignalColor()} p-4`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {getSignalIcon()}
            <div>
              <h3 className="text-xl font-bold">{signal.signal_type}</h3>
          ...
          <div className="text-right">
            <div className={`${getGradeColor()} px-3 py-1 rounded-full text-sm font-bold mb-1`}>
              {signal.grade}
            </div>
      ...
      <div className="p-6">
        <div className="flex items-center gap-4 text-sm text-slate-400 mb-4">
          ...
          <span>•</span>
          <span className="text-amber-400">{signal.strategy_used}</span>
          <span>•</span>
          <span>{signal.timeframe}</span>
          {signal.is_killzone && (
            <>
              <span>•</span>
              <span className="text-green-400">Killzone</span>
            </>
          )}
        </div>
        ...
        {signal.signal_type !== 'HOLD' && signal.entry_price && (
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="bg-slate-800/50 p-3 rounded-lg">
              ...
              <div className="text-lg font-semibold">{formatPrice(signal.entry_price)}</div>
            </div>
            <div className="bg-slate-800/50 p-3 rounded-lg">
